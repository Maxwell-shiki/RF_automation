...
-----
- 毕设完成
- 软著申请材料已完成，具体资料见本地电脑，该电脑 `.\patent` 文件夹仅供参考
- 准备编写测试系统的测试用例，先以LDO测试为例
- 测试流程参考[Tektronix 关于电源芯片PSRR的测量](https://mp.weixin.qq.com/s/uhZ3LtzWV8v_NHvqGDQnSA)
- 需要DC Power Supply, Line injector, MSO64B, Load + DUT
  - 看下 电子负载 ET5300A 的程控手册
  - 需要学下Line injector使用

2024.5.27
----
- 继续准备电子负载的自动化调试
  - 淘宝店家还没把软件发给我，官网有相近型号的Labview程控软件，先试着用下
- LDO PSRR的仪器测试搭起来了，但还得先把电子负载的Python自动化做了
- ET5300A 电子负载的自动化
  - 商家终于给到了上位机软件和SCPI协议说明书
  - SCPI主要是他串口指令后要加帧尾`0A`(就是ASCII的\n)，需要注意下
- 电子负载的自动化写完了，就是指令延时它不支持`OPC?`，只能自己毛估估0.8s
- 下次还要写点负载量程切换的问题

2024.5.30
----
- 补充电子负载的自动化模块
  - 模块目前支持模式有定电流CC、定电压CV、定电流转电压CCCV（待测源无法输出负载设定的电流值时，自动由CC→CV）
  - 暂不支持动态模式和列表模式，感觉实际需求不大，到时候再说
- 写了低温MOS建模的测试环境搭建脚本
  - ES3631A不支持从-0.5V~3.3V单通道的连续变化。故暂缓

2024.6.13
----
- 进行LDO测试样例的编写
  - [测试仪器链接图](https://whimsical.com/nicic-ldotest-NfruiJnU38oLuqXqnZ3VgF)参考
...
2024.6.28
----
- 准备发表ICSICT论文，A High PSRR LDO for Sensor Applications
- 继续写LDO测试样例

2024.7.2
----
- 论文前几天就写完了，已经交了。但截稿日期延到了7.15
- PSRR LDO先用示波器方案做一个，后续再用网络分析仪做一个
- 示波器方案精度不高，先大概看看